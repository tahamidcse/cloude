...
def runcode(request):
    if request.method == 'POST':
        ...
        with open("user/"+input_file, 'w') as f:
            f.write(input_part)
        ...
        try:
            # Compile the C++ code
            compile_process = subprocess.run(["g++", "user/" + file], input=code_part, text=True, capture_output=True)

            if compile_process.returncode != 0:
                raise Exception(compile_process.stderr)

            # Run the compiled program with input/output redirection
            run_process = subprocess.run(["a.exe"],  input=input_part, text=True, capture_output=True)

            output = run_process.stdout
            with open("user/"+output_file, 'w') as f:
                f.write(output)
